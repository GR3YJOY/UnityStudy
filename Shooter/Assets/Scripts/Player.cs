...
public class Player : MonoBehaviour
{
    ...
    void Update()  //움직일 때마다 화면에 갱신되어 나타나는 부분
    {
        Move();
        Fire();
        ReloadBullet();
    ...
    private void Move() //비행기 이동
    {
        float h = Input.GetAxisRaw("Horizontal");   //GetAxis : 방향을 정보를 알고싶을 때, Raw : 방향만 나오는 함수 (정수)
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        else if (collision.gameObject.tag == "ENEMYBULLET")
        {
            ...
            gameObject.SetActive(false); // 적총알에 맞았을 경우 플레이어를 죽이지 않고 화면 상으로만 안보이게 설정
